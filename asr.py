...
class ChunkBufferDecoder:
    ...
    def decode_final(self, merge=True, extra=0):
        ...
        print((self.chunk_len + (self.buffer_len - self.chunk_len) / 2))
        print(self.model_stride_in_secs)
        print("Delay: ",delay)
        decoded_frames = []
        all_toks = []
        ...
        for decoded in decoded_frames:
            ...
            for p in perbuffer:
                if (p != previous or previous == self.blank_id) and p != self.blank_id:
                    valid.append(p.item())
                    text=self.asr_model.tokenizer.ids_to_text([p.item()])
                    print(text,len(text),len(p.item()))
                    previous = p
            print("Frame: ",frame)
